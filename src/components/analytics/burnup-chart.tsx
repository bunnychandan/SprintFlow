...
import type { BurnupChart as BurnupChartType } from "@/types/analytics";

interface BurnupChartProps {
  data: BurnupChartType | null;
  loading?: boolean;
  className?: string;
}

export function BurnupChart({ data, loading, className }: BurnupChartProps) {
  if (loading) {
    return (
  ...
  if (!data || !data.data || data.data.length === 0) {
    return (
      <Card className={cn("p-5", className)}>
        <h4 className="text-sm font-semibold text-foreground-secondary uppercase tracking-wider mb-4">Burnup</h4>
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data.data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            <Line type="monotone" dataKey="total" name="Total Scope" stroke="#94a3b8" strokeWidth={2} dot={false} />
