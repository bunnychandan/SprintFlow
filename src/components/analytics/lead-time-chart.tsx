...
import type { LeadTimeChart as LTCType } from "@/types/analytics";
...
export function LeadTimeChart({ data, loading, className }: LeadTimeChartProps) {
  if (loading) {
    return (
  ...
  if (!data) {
    return (
      <Card className={cn("p-5", className)}>
        <h4 className="text-sm font-semibold text-foreground-secondary uppercase tracking-wider mb-4">Lead Time</h4>
      ...
      <div className="h-48">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data.distribution} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            />
            <Bar dataKey="value" name="Tasks" fill="#818cf8" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </Card>
