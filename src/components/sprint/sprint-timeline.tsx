...
import { Plus, Play, CheckCircle2, XCircle, ListPlus, ListX, RefreshCw } from "lucide-react";
import type { SprintTimelineEvent } from "@/types/sprint";
...
const typeColors: Record<string, string> = {
  ...
  task_added: "text-amber-500 bg-amber-500/10",
  task_removed: "text-rose-500 bg-rose-500/10",
  points_updated: "text-indigo-500 bg-indigo-500/10",
};

export function SprintTimeline({ events, loading }: SprintTimelineProps) {
  if (loading) {
    return (
  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border p-12 text-center">
        ...
        <p className="mt-1 text-sm text-foreground-secondary">Sprint activity will appear here.</p>
      </div>
    );
  }

  const groupedByDate: Record<string, SprintTimelineEvent[]> = {};
        ...
        <div key={dateKey} className="mb-6">
          ...
          <div className="space-y-3">
            {dateEvents.map((event) => {
              ...
              const colorClass = typeColors[event.type] || "text-slate-500 bg-slate-500/10";
