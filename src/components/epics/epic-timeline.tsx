...
import { Plus, LayoutDashboard, RefreshCw, Activity, ListPlus } from "lucide-react";
import type { EpicTimelineEvent } from "@/types/agile";
...
export function EpicTimeline({ events, loading }: EpicTimelineProps) {
  if (loading) {
    return (
  ...
  if (!events || events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border p-12 text-center">
        <LayoutDashboard className="h-12 w-12 text-foreground-muted mb-4" />
        <h3 className="text-lg font-semibold text-foreground">No Activity Yet</h3>
        <p className="mt-1 text-sm text-foreground-secondary">Epic activity will appear here.</p>
      </div>
    );
  }

  const groupedByDate: Record<string, EpicTimelineEvent[]> = {};
