...
import { Plus, Package, RefreshCw, Activity, ListPlus } from "lucide-react";
...
const typeColors: Record<string, string> = {
  ...
  published: "text-violet-500 bg-violet-500/10",
  cancelled: "text-red-500 bg-red-500/10",
};

export function ReleaseTimeline({ events, loading }: ReleaseTimelineProps) {
  if (loading) {
    return (
  ...
  if (!events || events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border p-12 text-center">
        <Package className="h-12 w-12 text-foreground-muted mb-4" />
        <h3 className="text-lg font-semibold text-foreground">No Activity Yet</h3>
        <p className="mt-1 text-sm text-foreground-secondary">Release activity will appear here.</p>
      </div>
    );
  }
