...
import type { ReleaseStatistics } from "@/types/agile";

interface ReleaseStatsProps {
  stats: ReleaseStatistics | null;
  loading?: boolean;
  className?: string;
}

export function ReleaseStatistics({ stats, loading, className }: ReleaseStatsProps) {
  if (loading) {
    return (
  ...
  const statCards = [
    ...
    { label: "Total Points", value: stats.totalPoints, icon: Target, color: "text-indigo-500" },
    { label: "Completed Points", value: stats.completedPoints, icon: TrendingUp, color: "text-violet-500" },
  ];
