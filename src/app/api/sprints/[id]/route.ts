...
export async function PUT(
  ...
  const sprintExists = await prisma.sprint.findUnique({ where: { id } });
  if (!sprintExists) {
...
export async function DELETE(
  ...
  await prisma.$transaction([
    ...
    prisma.auditLog.create({
      data: { actorId, entityType: "SPRINT", entityId: id, action: "DELETE_SPRINT", details: `Deleted sprint ${sprint.name}` },
    }),
  ]);
